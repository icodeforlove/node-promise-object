...
Class.create = function () {
		...
		instance = function (cookie) {
			...
			self.reopen = function (properties) {
				for (var property in properties) {
					...
					self[property] = _.isFunction(self[property]) || self[property] instanceof ExtendedMethod ? mapMethod(self, property, self[property]) : self[property];
				}
			};

			if (cookie !== isFunction && (isFunction(this.initialize) || this.initialize instanceof ExtendedMethod)) {
				for (var method in this) if (method.substr(0, 1) !== '$' && (this[method] instanceof ExtendedMethod || isFunction(this[method]))) {
	...
	for (key in instance.prototype) {
		if (key.substr(0, 1) === '$') {
			instance[key.substr(1)] = _.isFunction(instance.prototype[key]) || instance.prototype[key] instanceof ExtendedMethod ? mapMethod(instance.prototype, key, instance.prototype[key]) : instance.prototype[key];
		}
	}
	...
	instance.reopen = function (properties) {
		for (var property in properties) {
			...
			instance[property] = _.isFunction(instance.prototype['$' + property]) || instance.prototype['$' + property] instanceof ExtendedMethod ? mapMethod(instance, property, instance.prototype['$' + property]) : instance.prototype['$' + property];
		}
	};
